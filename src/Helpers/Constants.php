<?php

namespace App\Helpers;

abstract class Constants
{
    public const ERROR_METHOD_NOT_ALLOWED = 'Method Not Allowed.';
    public const ERROR_DENIED_ACCESS = 'Access denied. You do not have permission to access this resource.';
    public const ERROR_INVALID_FORMAT = 'Invalid input format.';
    public const ERROR_NOT_FILE_UPLOADED = 'No file uploaded.';
    public const ERROR_EVENT_DOESNT_EXITS = 'Event not found.';
    public const ERROR_USER_DOESNT_EXITS = 'User not found.';
}